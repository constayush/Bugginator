...
const [activeTab, setActiveTab] = useState(null);
  ...
  return (
    <div className="flex min-h-screen w-full h-full relative">
      {/* Sidebar */}
      <aside className="w-20 pt-[3rem] md:w-60 text-[var(--primary-text-color)]  sticky self-start top-0 border-r bg-[#9d00ff0c] h-screen border-gray-300 dark:border-gray-700 flex flex-col p-4 space-y-4">
    ...
    <h1 className="text-2xl">Bugginator</h1>
        ...
        {sidebarItems.map((item) => (
          <Link
            ...
            ${
              activeTab === item.id
                ? "bg-purple-500/15 border-[var(--navbar-bg-color)] border"
                : "bg-transparent"}
            `}
          >
